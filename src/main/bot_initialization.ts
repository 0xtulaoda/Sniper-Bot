...
function welcome() {
  ...
  console.log(
    fs.readFileSync(path.join(__dirname, '..', '..', 'assets', 'credits.ans')).toString(),
  );
  console.log('\x1b[0m');
}
...
function showCurrentBotSettings() {
  ...
  console.log(
    '\x1b[33m' + language.lang.MODE + ': \x1b[32m%s',
    mode.get('fairlaunch') ? 'fairlaunch' : 'presale',
  );
  console.log(
    '\x1b[33m' + language.lang.DELAY + ': \x1b[3%dm%s',
    mode.get('delay') ? 2 : 1,
    mode.get('delay') ? 'on (' + mode.get('delay') + ' ' + language.lang.BLOCKS + ')' : 'off',
  );
  console.log(
    '\x1b[33m' + language.lang.NET + ': \x1b[3%sm%s',
    mode.get('testnet') ? 1 : 2,
    (mode.get('testnet') ? 'testnet' : 'mainnet') + '\x1b[0m',
  );
}
