...
class CommsHandler {
  ...
  constructor(
    ...
    gas_price: string = '5',
    gas_amount: string = '500000',
    amount: string,
  ) {
    this.HTTP_PROVIDER = useTestnet
  ...
  public setTargetContract(target_contract: string): void {
    if (!this.web3.utils.isAddress(target_contract)) throw new Error(language.lang.ADDR_NOT_VALID);
  ...
  public addPrivateKeys(private_keys: string[]): void {
    private_keys.forEach((key: string) => {
      if (!this.verifyPrivateKey(key)) throw new Error(language.lang.PVT_KEY_NOT_VALID);
    });
  ...
  public sendTXs(callback: Function): void {
    this.signed_tx.forEach(async (sig_tx: any) => {
      this.web3.eth.sendSignedTransaction(
        (await sig_tx).rawTransaction,
        (error: Error, result: any) => {
          if (!callback) this.defaultCallback(error, result);
