...
class logger {
  ...
  private bot_settings: {
    settings: Map<string, boolean | number>;
  ...
  private readonly update = (function () {
    ...
    return function (
      error?: { error: Error; caught: boolean; timestamp: Logger.LoggingTimestamp },
      exiting?: boolean,
    ) {
      ...
      if (error) {
        if (error.caught) {
          ...
          fs.appendFileSync(
            this.full_path,
            error.error.stack ? error.error.stack : 'stack not available.',
          );
          fs.appendFileSync(this.full_path, '\n');
      ...
      if (!isBotSettingsDisplayed) {
        if (this.bot_settings) {
          fs.appendFileSync(
            this.full_path,
            this.bot_settings.notification_time + '\t' + 'Bot started with following setting: \n',
          );
          fs.appendFileSync(
            this.full_path,
            'Mode: ' +
              (this.bot_settings.settings.get('fairlaunch') ? 'fairlaunch' : 'presale') +
              '\n',
          );
          fs.appendFileSync(
            this.full_path,
            'Delay: ' +
              (this.bot_settings.settings.get('delay')
                ? 'on (' + this.bot_settings.settings.get('delay') + ' ' + 'blocks' + ')'
                : 'off') +
              '\n',
          );
          fs.appendFileSync(
            this.full_path,
            'Mode: ' + (this.bot_settings.settings.get('testnet') ? 'testnet' : 'mainnet') + '\n',
          );
          isBotSettingsDisplayed = true;
        } else throw new Error('No bot settings data (and further) available.');
      } else if (!isWalletConfigDisplayed) {
        if (this.wallet_config) {
          fs.appendFileSync(
            this.full_path,
            this.wallet_config.notification_time +
              '\t' +
              'Wallet configuration is the following: \n',
          );
          fs.appendFileSync(this.full_path, 'Private key: ' + '[CENSORED]\n');
          fs.appendFileSync(
            this.full_path,
            'Gas amount: ' + this.wallet_config.configuration.get('gas_amount') + '\n',
          );
          fs.appendFileSync(
            this.full_path,
            'Gas price: ' + this.wallet_config.configuration.get('gas_price') + '\n',
          );
          fs.appendFileSync(
            this.full_path,
            'Amount: ' + this.wallet_config.configuration.get('amount') + '\n',
          );
          isWalletConfigDisplayed = true;
        } else throw new Error('No wallet settings data (and further) available.');
      } else if (!isTargetAddressDisplayed) {
        if (this.target_address) {
          fs.appendFileSync(
            this.full_path,
            this.target_address.notification_time +
              '\t' +
              'Target address: ' +
              this.target_address.target_address,
          );
          isTargetAddressDisplayed = true;
        } else throw new Error('No target address (and further) available.');
      } else if (!isTargetTimeDisplayed) {
        if (this.target_time) {
          fs.appendFileSync(
            this.full_path,
            this.target_time.notification_time +
              '\t' +
              'Target time: ' +
  ...
  private clearSensibleData() {
    if (this.wallet_config && this.wallet_config.configuration.has('private_key')) {
      let private_key_regex: RegExp = new RegExp(
        `${this.bot_settings.settings.get('private_key')}`,
        'g',
      );
  ...
  private constructor() {
    ...
    fs.appendFileSync(
      this.full_path,
      `===LOG FILE CREATED AT TIMESTAMP ${this.filetimestamp}===\n\n`,
    );
    ...
    function exitHandler(exitCode: number) {
      ...
      //other codes (external entities called process.exit())
        ...
        this.update(
          {
            error: new Error(
              `exit(${exitCode}) has been called. Program forcibly closed by external entity.`,
            ),
            caught: false,
            timestamp: Logger.LoggingTimestamp.getTimestamp(),
          },
    ...
    function sigHandler(sig: any) {
      this.update(
        {
          error: new Error(
            `Sig (${sig.toString()}) has been called. Program forcibly closed by external entity.`,
          ),
          caught: false,
          timestamp: Logger.LoggingTimestamp.getTimestamp(),
        },
        ...
        `[${now.getHours() < 10 ? '0' + now.getHours().toString() : now.getHours()}:` +
          ...
          `${
            now.getMilliseconds() < 10
              ? '000' + now.getMilliseconds().toString()
              : now.getMilliseconds() < 100
              ? '00' + now.getMilliseconds().toString()
  ...
  export class FileTimestamp extends Object {
    ...
    public static getTimestamp(): FileTimestamp {
      ...
      return new FileTimestamp(
        `${now.getDay() < 10 ? '0' + now.getDay().toString() : now.getDay().toString()}` +
